...
@Component({
	template: `
		<button class="btn btn-primary mb-3" id="toggleDrawer" (click)="toggleDrawer()">Toggle drawer</button>
		<div class="d-flex align-items-center">
			<label for="drawerPlacement" class="me-3">Placement:</label>
			<select id="drawerPlacement" [ngModel]="drawerPlacement()" (ngModelChange)="drawerPlacement.set($event)" class="w-auto form-select">
